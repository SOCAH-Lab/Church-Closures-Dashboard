...
During the data cleaning and validation steps (**Step 5** for the 2023 Format and **Step 2 LOOP PART D** for the 2026 Format) longitude and latitude coordinates were used to assign census boundary designations across the 2000, 2010, and 2020 Decennial Census years. The initial implementation of this spatial assignment method used the `tigris` R package to retrieve decennial-year shapefiles directly from the U.S. Census Bureau's [TIGER/Line Shapefile](https://www.census.gov/geographies/mapping-files/time-series/geo/tiger-line-file.html) database.

This approach, however, did not scale effectively and introduced processing issues with the 2000 and 2020 decennial years. As a result, and given the time constraints leading up to the Summer 2025 Symposium, the 2010 Census Bureau ZCTA-to-County [Relationship File](https://www.census.gov/geographies/reference-files/time-series/geo/relationship-files.2010.html#par_textimage_674173622) consistent with the source used by Dr. Insang Song, was used to annotate census boundaries by joining on the address ZIP code.
...
During development of "Generate the Metrics_2023 Format.R", **Step 5** was still running and did not have sufficient results to proceed with. While Step 5 continued to run in the background, **Step 4** results, which lacked census boundary annotations, were used to develop the script. As a temporary measure, the 2010 ZCTA-to-County crosswalk file, `zcta_county_rel_10.txt,` was used to annotate ZIP codes with county and state FIPS codes in the **Step 4** results. The crosswalk data source is consistent with that used by Dr. Insang Song in his version of this process. Once sufficient **Step 5** results were available, the crosswalk file was no longer needed and was otherwise not used.
...
6.  `zcta_county_rel_10.txt` and `explanation_zcta_county_rel_10.pdf` copied Summer 2025 from Dr. Insang Song's GitHub repository for the project: [sigmafelix/healthreligion_project](https://github.com/sigmafelix/healthreligion_project) and also available directly from the 2010 Census Bureau ZCTA-to-County [Relationship File](https://www.census.gov/geographies/reference-files/time-series/geo/relationship-files.2010.html#par_textimage_674173622) page.
