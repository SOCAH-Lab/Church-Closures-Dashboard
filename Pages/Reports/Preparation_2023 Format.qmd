...
One other notable limitation warrants mention. The similarity between verified and reported geocoordinates was overly relied upon; for some addresses, the two differed enough to warrant using the verified coordinates over the raw, reported values. Both of these fundamental limitations were addressed in the revised 2026 formatted version pipeline.
...
Gross location changes are classified by the maximum difference in longitude and latitude coordinates listed across all addresses for a given business. To get a sense of maximal location change, a threshold of 0.001 degrees is applied uniformly to both coordinate differences, which approximates one city block.

Note that the relationship between degrees and Euclidean distance differs between longitude and latitude and varies by location on the Earth's surface. This check intentionally simplifies those details to provide a quick and broad, high-level sense of address movement for a given business. For reference, 0.002 degrees $\approxeq$ 222 m, given that there are approximately 111,000 m per degree.
...
Nearly 86\% of businesses remained within one block of each other. An additional 12\% had a relocation within five miles, 1.6% between five and ten miles, and less than 1% moved more than ten miles, of which 0.08% moved more than 50 miles. Table \ref{tbl-confirm_dim_clusters_1} parses clustering results by number of unique addresses associated with a business and by maximum longitude/latitude coordinate difference detected.
...
\addlinespace[10pt]
\texttt{latitude/longitude} & The longitude/latitude of the selected address. The assumption was that verified geographies closely corresponded with these coordinates. This was later found to be incorrect, and the verified geocoordinates were instead used in later iterations of this pipeline. \\
\addlinespace[10pt]
